...
func runRepoNew(ctx context.Context, env *Env, args []string) error {
	...
	if !*noRemote {
		created, err := createRemote(ctx, env, dir, name, originURL, *private)
		if err != nil {
...
func createRemote(ctx context.Context, env *Env, dir, name, originURL string, private bool) (string, error) {
	if originURL != "" {
...
func runRepoAdopt(ctx context.Context, env *Env, args []string) error {
	...
	discovered, ok, err := describeRepo(ctx, ws.Root, name)
	if err != nil {
		return err
	}
	if !ok {
...
func describeRepo(ctx context.Context, root, name string) (manifest.Repo, bool, error) {
	dir := filepath.Join(root, name)
	if !gitx.IsRepository(dir) {
		return manifest.Repo{}, false, nil
	}
...
func confirm(env *Env, question string) bool {
	fmt.Fprintf(env.Printer.Out(), "%s [y/N] ", question)
