...
func adapterPreamble(role Role) string {
	var b strings.Builder
	b.WriteString(fmt.Sprintf("Read `%s/%s.md` in full before acting, and follow that contract.\n",
		RolesDir, role.Name))
	...
	if len(role.Writes) == 0 {
		...
		b.WriteString(fmt.Sprintf("Write only inside: %s. Every other repository is read-only.\n",
