...
func checkUnreferencedBrain(ws *workspace.Workspace) []Finding {
	...
	for _, entry := range entries {
		...
		dir := filepath.Join(ws.Root, entry.Name())
		if hasBrain && filepath.Clean(declared) == dir {
			continue
		}
		if !fsx.Exists(filepath.Join(dir, brain.MarkerFile)) {
			continue
		}
		fix := fmt.Sprintf("vat brain adopt %s", entry.Name())
		if _, governed := ws.Manifest.Find(entry.Name()); !governed {
			fix = fmt.Sprintf("vat repo adopt %s, then vat brain adopt %s", entry.Name(), entry.Name())
		}
		findings = append(findings, Finding{
			Rule: "brain/unreferenced", Severity: SeverityWarn, Subject: entry.Name(),
