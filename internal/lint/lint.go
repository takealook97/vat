...
func Run(ctx context.Context, ws *workspace.Workspace, opts Options) (Report, error) {
	...
	repoFindings, err := checkRepositories(ctx, ws)
	if err != nil {
		return report, err
	}
	add(repoFindings...)
...
func checkRepositories(ctx context.Context, ws *workspace.Workspace) ([]Finding, error) {
...
func checkHarness(ws *workspace.Workspace) ([]Finding, error) {
	...
	case !exists:
		findings = append(findings, Finding{
			...
			Fix:     "vat harness render", Fixable: true,
		})
