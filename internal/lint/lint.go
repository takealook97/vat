...
func checkHarness(ws *workspace.Workspace) ([]Finding, error) {
	...
	for _, role := range roles {
		...
		findings = append(findings, unknownRuntimes(role.Name, role.Runtimes)...)
...
//
// `runtimes:` decides which adapters exist, and an unrecognised value — a typo
// for `claude`, or a runtime vat does not support yet — silently produces none.
// Nothing else notices: there is no adapter, so there is no drift; the metadata
// is present, so role-metadata passes; and a bare `model` binds to nothing,
// so model-ambiguous passes too. The definition sits on disk, inert, while
// every diagnostic reports the harness healthy.
func unknownRuntimes(subject string, declared []string) []Finding {
	var findings []Finding
	for _, name := range declared {
		known := false
		for _, runtime := range harness.RuntimeNames() {
		...
		findings = append(findings, Finding{
			...
			Message: fmt.Sprintf("declares runtime %q, which generates no adapter; it is one of %s or nothing at all",
				name, strings.Join(harness.RuntimeNames(), ", ")),
