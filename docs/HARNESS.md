...
drift for the same reason and are kept honest the same way.

```text
.agents/skills/release-a-service/
  SKILL.md            the procedure — canonical, written once
  references/         anything it cites
...
Read `.agents/skills/release-a-service/SKILL.md` in full and follow it exactly.
```

Copying the procedure into each runtime directory is the failure this prevents.
Two copies of a release skill that disagree by one step are worse than one copy
nobody has read, because both look authoritative and neither is dated.

A skill directory with no `SKILL.md` is references, not a skill, and is ignored
rather than reported. A skill with no `description` **is** reported: it is on
